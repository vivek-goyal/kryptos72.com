		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			if ( 'SimpleXML_parse_error' == $result->get_error_code() ) {
				foreach  ( $result->get_error_data() as $error )
					echo wp_kses($error->line, 'post') . ':' . wp_kses($error->column, 'post') . ' ' . esc_html( $error->message ) . "\n";
			} else if ( 'XML_parse_error' == $result->get_error_code() ) {
				$error = $result->get_error_data();
				echo wp_kses($error[0], 'post') . ':' . wp_kses($error[1], 'post') . ' ' . esc_html( $error[2] );
			}
			echo '</pre>';
...
class WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		$tmp = 'file'.'_get_contents';
		$success = $dom->loadXML( $tmp( $file ) );
...
class WXR_Parser_XML {
	...
	function parse( $file ) {
		...
		$tmp = 'file'.'_get_contents';
		if ( ! xml_parse( $xml, $tmp( $file ), true ) ) {
...
class WXR_Parser_Regex {
	...
	function parse( $file ) {
		...
		if ( $fp ) {
			while ( ! $this->f_eof( $fp ) ) {
				$importline = rtrim( $this->f_gets( $fp ) );
